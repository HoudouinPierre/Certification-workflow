...
class GaussianProcessSKlearn:
    ...
    def _create_kernel(self):
        ...
        self._kernel             = 1 * sk_kernels.RBF(length_scale=1.0)
        self._kernel.k1          = sk_kernels.ConstantKernel(constant_value=sigma0 ** 2, constant_value_bounds=(lower_bound_sigma0 ** 2, upper_bound_sigma0 ** 2))
        if self._kernel_name == "RBF":
            self._kernel.k2 = sk_kernels.RBF(length_scale=length_scale, length_scale_bounds=(lower_bound_length_scale, upper_bound_length_scale))
        if self._kernel_name == "Exponential":
            self._kernel.k2 = sk_kernels.Matern(nu=0.5, length_scale=length_scale, length_scale_bounds=(lower_bound_length_scale, upper_bound_length_scale))
        if self._kernel_name == "Matern 3/2":
            self._kernel.k2 = sk_kernels.Matern(nu=1.5, length_scale=length_scale, length_scale_bounds=(lower_bound_length_scale, upper_bound_length_scale))
        if self._kernel_name == "Matern 5/2":
            self._kernel.k2 = sk_kernels.Matern(nu=2.5, length_scale=length_scale, length_scale_bounds=(lower_bound_length_scale, upper_bound_length_scale))
        self._kernel = self._kernel + sk_kernels.WhiteKernel(noise_level=noise, noise_level_bounds=(lower_bound_noise, upper_bound_noise))

    def _initialize_gaussian_process_model(self):
        n_restarts_optimizer         = self._kernel_parameters_optimization["SKlearn"]["N restarts optimizer"]
        self._gaussian_process_model = sk_gp.GaussianProcessRegressor(kernel=self._kernel, n_restarts_optimizer=n_restarts_optimizer)

    def _update_gaussian_process_model_kernel(self):
        sigma0                                     = self._kernel_parameters_optimization["sigma0"      ]
        length_scale                               = self._kernel_parameters_optimization["Length scale"]
        noise                                      = self._kernel_parameters_optimization["Noise"       ]
        self._gaussian_process_model.kernel_.k1.k1 = sk_kernels.ConstantKernel(constant_value=sigma0 ** 2)
        if self._kernel_name == "RBF":
            self._gaussian_process_model.kernel_.k1.k2 = sk_kernels.RBF(length_scale=length_scale)
        if self._kernel_name == "Exponential":
            self._gaussian_process_model.kernel_.k1.k2 = sk_kernels.Matern(nu=0.5, length_scale=length_scale)
        if self._kernel_name == "Matern 3/2":
            self._gaussian_process_model.kernel_.k1.k2 = sk_kernels.Matern(nu=1.5, length_scale=length_scale)
        if self._kernel_name == "Matern 5/2":
            self._gaussian_process_model.kernel_.k1.k2 = sk_kernels.Matern(nu=2.5, length_scale=length_scale)
        self._gaussian_process_model.kernel_.k2 = sk_kernels.WhiteKernel(noise_level=noise)
